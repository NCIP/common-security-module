...
public class LoginAction extends Action implements Constants {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		AuthenticationManager authenticationManager = SecurityServiceProvider.getAuthenticationManager(CSM_RI_CONTEXT_NAME);
		if (authenticationManager.login( loginForm.getLoginID(), loginForm.getPassword())) {
			request.getSession().setAttribute(USER,EmployeeDAO.searchEmployeeByUserName(loginForm.getLoginID()).get(0));
