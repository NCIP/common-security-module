...
BEGIN
  ...
  ELSIF UPDATING THEN
	...
	IF :new.PROTECTION_TYPE_ID IS NULL THEN
	  newTempRec := TO_CHAR(:old.PROTECTION_TYPE_ID) || '|';
	ELSE
	  newTempRec := TO_CHAR(:new.PROTECTION_TYPE_ID) || '|';
        END IF;

	IF :new.PROTECTION_TYPE_NAME IS NULL THEN
	  newTempRec := newTempRec || :old.PROTECTION_TYPE_NAME;
	ELSE	
	  newTempRec := newTempRec || :new.PROTECTION_TYPE_NAME;
	END IF;
