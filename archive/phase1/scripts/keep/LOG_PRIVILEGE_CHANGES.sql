...
BEGIN
  ...
  ELSIF UPDATING THEN
	...
	IF :new.PRIVILEGE_ID IS NULL THEN
	  newTempRec := TO_CHAR(:old.PRIVILEGE_ID) || '|';
	ELSE
	  newTempRec := TO_CHAR(:new.PRIVILEGE_ID) || '|';
        END IF;

	IF :new.PRIVILEGE_NAME IS NULL THEN
	  newTempRec := newTempRec || :old.PRIVILEGE_NAME || '|';
	ELSE	
	  newTempRec := newTempRec || :new.PRIVILEGE_NAME || '|';
	END IF;

	IF :new.PRIVILEGE_DESC IS NULL THEN
	  newTempRec := newTempRec || :old.PRIVILEGE_DESC || '|';
	ELSE	
	  newTempRec := newTempRec || :new.PRIVILEGE_DESC || '|';
	END IF;
