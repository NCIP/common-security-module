...
BEGIN
  ...
  ELSIF UPDATING THEN
	...
	IF :new.APPLICATION_ID IS NULL THEN
	  newTempRec := TO_CHAR(:old.APPLICATION_ID) || '|';
	ELSE
	  newTempRec := TO_CHAR(:new.APPLICATION_ID) || '|';
        END IF;

	IF :new.APPLICATION_NAME IS NULL THEN
	  newTempRec := newTempRec || :old.APPLICATION_NAME || '|';
	ELSE	
	  newTempRec := newTempRec || :new.APPLICATION_NAME || '|';
	END IF;

	IF :new.APPLICATION_DESC IS NULL THEN
	  newTempRec := newTempRec || :old.APPLICATION_DESC || '|';
	ELSE	
	  newTempRec := newTempRec || :new.APPLICATION_DESC || '|';
	END IF;

	IF :new.DECLARATIVE_FLAG IS NULL THEN
	  newTempRec := newTempRec || :old.DECLARATIVE_FLAG || '|';
	ELSE	
	  newTempRec := newTempRec || :new.DECLARATIVE_FLAG || '|';
	END IF;
