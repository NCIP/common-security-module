...
BEGIN
  ...
  ELSIF UPDATING THEN
	...
	IF :new.PROTECTION_GROUP_ID IS NULL THEN
	  newTempRec := TO_CHAR(:old.PROTECTION_GROUP_ID) || '|';
	ELSE
	  newTempRec := TO_CHAR(:new.PROTECTION_GROUP_ID) || '|';
        END IF;

	IF :new.PROTECTION_GROUP_NAME IS NULL THEN
	  newTempRec := newTempRec || :old.PROTECTION_GROUP_NAME || '|';
	ELSE	
	  newTempRec := newTempRec || :new.PROTECTION_GROUP_NAME || '|';
	END IF;

	IF :new.PROTECTION_GROUP_DESC IS NULL THEN
	  newTempRec := newTempRec || :old.PROTECTION_GROUP_DESC || '|';
	ELSE	
	  newTempRec := newTempRec || :new.PROTECTION_GROUP_DESC || '|';
	END IF;

	IF :new.LARGE_ELEMENT_COUNT_FLAG IS NULL THEN
	  newTempRec := newTempRec || :old.LARGE_ELEMENT_COUNT_FLAG || '|';
	ELSE	
	  newTempRec := newTempRec || :new.LARGE_ELEMENT_COUNT_FLAG || '|';
	END IF;
	...
	IF :new.PROTECTION_GROUP_TYPE_ID IS NULL THEN
	  newTempRec := newTempRec || TO_CHAR(:old.PROTECTION_GROUP_TYPE_ID);
	ELSE	
	  newTempRec := newTempRec || TO_CHAR(:new.PROTECTION_GROUP_TYPE_ID);
	END IF;
