 ...
 * 
 *  Features include:
 *	--Inserts the SQL statements into the database in near real time
 *	--Uses a configurable buffer to perform batch processing
 * 	--Spawns threads to execute the batch inserts to maximize performance
...
public class JDBCAppender extends AppenderSkeleton implements Constants {
    ...
    public void append(LoggingEvent le) {
        ...
        sql.append( APPLICATION ); sql.append( "," );
        sql.append( SERVER ); sql.append( "," );
        sql.append( PRIORITY ); sql.append( "," );
        sql.append( CATEGORY ); sql.append( "," );
        sql.append( THREAD ); sql.append( "," );
        sql.append( MSG ); sql.append( "," );
        sql.append( THROWABLE ); sql.append(",");
        sql.append( NDC ); sql.append( "," );
        sql.append( CREATED_ON ); sql.append(") VALUES ('");
        sql.append( clean( getApplication() ) ); sql.append( "','" );
        sql.append( clean( getServer() ) ); sql.append("','" );
        String level = "";
        if ( le.getLevel() != null ){
        	level = le.getLevel().toString();
        }
        sql.append( clean( level ) ); sql.append("','");
        sql.append( clean( le.getLoggerName() ) ); sql.append("','");
        sql.append( clean( le.getThreadName() ) ); sql.append("','");
        String msg = "";
        if ( le.getMessage() != null ){
        	msg = le.getMessage().toString();
        }
        sql.append( clean( msg )); sql.append("','");
        sql.append( clean( getThrowable( le ) ) ); sql.append("','");
        sql.append( clean( le.getNDC() ) ); sql.append("',");
        sql.append( System.currentTimeMillis() ); sql.append( ")" );
	...
	/**
	 * @param recordCtr The recordCtr to set.
	 */
