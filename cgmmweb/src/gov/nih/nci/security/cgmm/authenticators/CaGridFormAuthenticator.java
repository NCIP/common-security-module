package gov.nih.nci.security.cgmm.authenticators;
 ...
 * 
 * This class is extended based on the FormAuthenticator. 
 *
 *
 
 */

public class CaGridFormAuthenticator
    extends AuthenticatorBase {
    
    
	  private static Log log = LogFactory.getLog(CaGridFormAuthenticator.class);
    ...
    protected static final String info =
        "gov.nih.nci.security.cgmm.authenticators.CaGridFormAuthenticator/1.0";
    ...
    public boolean authenticate(Request request,
        ...
        if (!loginAction) {
            ...
            try {
    			...
    			if (log.isDebugEnabled())
                    log.debug("Successfully retrieved Authentication Source Map from CGMM in session '" + session.getIdInternal() + "'");		    			
    		} catch (CGMMConfigurationException e) {
    			if (log.isDebugEnabled())
                    log.debug("Unable to retrieve Authentication Source Map from CGMM in session '" + session.getIdInternal() + "'");		
        ...
        if(null ==authenticationServiceURL){
        	...
        	if(null==performDefaultFormAuthentication){ 
        		...
        		if(performDefaultFormAuthentication.equalsIgnoreCase("YES")){
        			...
        			if (log.isDebugEnabled())
                        log.debug("CaGridFormAuthenticator: Default Form Authenticator functionality is requested.");
        		}
        	}
        }else{
    ...
    protected boolean restoreRequest(Request request, Session session)
        ...
        if ("POST".equalsIgnoreCase(saved.getMethod())) {
            ...
            if (body != null) {
                ...
                String savedContentType = null;//saved.getContentType();
