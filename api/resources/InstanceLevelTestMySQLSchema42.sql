...
# Replace instance42 with your application name.
#
...
insert into Card (id,name) select i+1, concat('Card',  i) from numbers where i < 50000;
insert into Suit (id,name) select i+1, concat('Suit',  i) from numbers where i < 10;
insert into Deck (id,name) select i+1, concat('Deck',  i) from numbers where i < 10;



insert into csm_user (user_id,login_name,first_name,last_name) select i+2, concat('User',  i),'a','b' from numbers where i < 50;
...
insert into csm_user_group_role_pg (group_id,protection_group_id, role_id ) 
	select g.group_id,pg.protection_group_id, r.role_id from csm_group g, csm_protection_group pg, csm_role r where r.role_name in ('READ', 'WRITE', 'ACCESS') ORDER By rand();
